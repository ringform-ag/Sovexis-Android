...
class KeyManagerImpl @Inject constructor(
        ...
        return try {
            ...
            if (isStrongBoxAvailable()) {
                try {
                    ...
                    // 超时保护：部分 OEM StrongBox Keymaster HAL 存在阻塞 bug
                    return withTimeout(10_000L) {
                        keyPairGenerator.generateKeyPair()
                    }
                } catch (e: Exception) {
                    // StrongBox 不可用或超时，回退到 TEE
                    builder.setIsStrongBoxBacked(false)
                }
            }

            // 使用 TEE — 重建实例避免前次初始化残留状态
            val fallbackGenerator = KeyPairGenerator.getInstance(
                KEY_ALGORITHM,
                ANDROID_KEYSTORE
            )
            fallbackGenerator.initialize(builder.build())
            withTimeout(10_000L) {
    ...
    fun derivePublicKeyFromPrivate(privateKey: PrivateKey): PublicKey {
        ...
        val publicKeySpec = org.spongycastle.jce.spec.ECPublicKeySpec(point, spec)
